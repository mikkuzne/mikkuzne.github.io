...
- Co-developed **EV-AUDIT**, a **co-evolutionary red/blue-team** auditing framework for multi-agent systems — attacks and defenses evolve against each other by reasoning over execution traces. It targets *task hijacking*: injections that quietly redirect an agent toward an attacker-chosen task inside its own domain, where there is no malicious verb to refuse. The procedure reached **31.26% attack success** on a frontier backend where all 22 baseline attacks fail entirely, and yielded a prompt-level defense that suppresses both baseline and evolved attacks at negligible utility cost.
